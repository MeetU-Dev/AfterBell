...
interface UserDataContextType {
    ...
    completeSkill: (skillId: string, steps: number[], timeSpent: number, score?: number, skillName?: string, totalSteps?: number) => void;
...
export const UserDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // Load user data on mount (localStorage cache + API sync)
    useEffect(() => {
        if (user) {
            loadUserData();
            syncFromApi();
    ...
    // Fetch all user progress from backend and merge into state
    const syncFromApi = async () => {
        if (!user) return;
        try {
            const data: any = await apiRequest('/api/v1/progress/sync');
            ...
            if (data.completions?.length) {
                setCompletedSkills(prev => {
                    ...
                    for (const c of data.completions) {
                        const idx = merged.findIndex(m => m.skillId === c.skillId);
                        const entry: CompletedSkill = {
                            skillId: c.skillId,
            ...
            if (data.bookmarks?.length) {
                setBookmarkedSkills(prev => {
                    ...
                    for (const b of data.bookmarks) {
                        const idx = merged.findIndex(m => m.skillId === b.skillId);
                        const entry: BookmarkedSkill = {
                            skillId: b.skillId,
            ...
            if (data.activities?.length) {
                setActivities(data.activities.slice(0, 10).map((a: any) => ({
                    id: a._id,
                    action: a.action,
                    skill: a.skill,
                    skillId: a.skillId,
    ...
    const saveUserData = () => {
        if (!user) return;
        const userKey = `afterbell_userdata_${user.id}`;
        const data = { profile, stats, achievements, activities, bookmarkedSkills, completedSkills };
    ...
    const completeSkill = (skillId: string, completedSteps: number[], timeSpent: number, score?: number, skillName?: string, totalSteps?: number) => {
        ...
        if (existingCompletion) {
            ...
            const newCompletion: CompletedSkill = {
                ...
                completedSteps,
                totalSteps: totalSteps || completedSteps.length,
                timeSpent,
                score
            };
        ...
        setStats(prev => ({
            ...prev,
            completedSkills: prev.completedSkills + (isNewCompletion ? 1 : 0),
            totalHours: prev.totalHours + (timeSpent / 60)
        }));
    ...
    useEffect(() => {
        checkAchievements();
    }, [completedSkills, bookmarkedSkills, stats, activities]);
    ...
    const bookmarkSkill = (skillId: string) => {
        ...
        apiRequest('/api/v1/progress/bookmark', {
            method: 'POST',
    ...
    const unbookmarkSkill = (skillId: string) => {
        ...
        apiRequest(`/api/v1/progress/bookmark/${skillId}`, {
            method: 'DELETE',
        }).catch(() => {});
    };
    ...
    const addActivity = (action: string, skill: string, skillId: string) => {
        ...
        apiRequest('/api/v1/progress/activity', {
            method: 'POST',
            body: JSON.stringify({ action, skill, skillId }),
        }).catch(() => {});
    };
